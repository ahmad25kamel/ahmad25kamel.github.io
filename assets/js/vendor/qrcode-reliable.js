/*!
 * QR Code Generator - Reliable Implementation
 * Based on proven QR code generation algorithms
...
(function() {
    ...
    // QR Code generator using proven algorithms
    ...
    window.QRCode = {
        toCanvas: function(text, options, callback) {
            ...
            try {
                const qr = new QRCodeLib(version, QRErrorCorrectLevel[errorLevel]);
                qr.addData(text);
                qr.make();
                
                const canvas = document.createElement('canvas');
                const moduleCount = qr.getModuleCount();
                const cellSize = Math.floor(width / (moduleCount + 2)); // +2 for margin
                const margin = Math.floor((width - cellSize * moduleCount) / 2);
